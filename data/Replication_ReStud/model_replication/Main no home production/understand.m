...
fprintf('\n')
fprintf('Fraction borrowers that truly benefit from refinance         = %9.3f %9.3f\n',    [sum(good(:))/sum(borr(:)), sum(goodc(:))/sum(borrc(:))]);
fprintf('Mean gains (units of c) of those that benefit                = %9.3f %9.3f\n',    [mean(Vgap(good)),          mean(Vgapc(goodc))]);
fprintf('Proportion of those that benefit who refinance               = %9.3f %9.3f\n',    [mean(Refi(good)),          mean(Refic(goodc))]);
...
fprintf('Mean gains (PV savings) of those that benefit                = %9.3f\n',    mean(PVgains(goodc)));
fprintf('Proportion of those that benefit who refinance               = %9.3f\n',    mean(Refic(goodc)));
