...
p.R          = x(3); 
p.F0m        = x(4);                              % fixed cost of refinancing
p.B          = x(5);
p.r1         = x(6); 
p.r2         = x(7);                              % parameters governing shape of rl curve
p.rh         = (1 + x(8))^(1/4) - 1;              % interest rate

% Assigned Parameters

p.nu         = 3;                                 % parameter in exponential distribution

p.phi        = 1e-7;                              % productivity non-market
...
lmin         = 1e-3;                                                               % keep it reasonably negative so they know that's a bad state to find yourself in 
lmax         = 125;
