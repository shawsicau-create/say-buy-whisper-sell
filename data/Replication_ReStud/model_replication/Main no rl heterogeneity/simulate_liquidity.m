...
for time = 2 : S
  ...
  state              = (1 + p.rl).*Asim(rent, initage,  time);
 ...
 % Homeowners
  ...
  Attemp             = (1 + p.rl).*Asim(~rent, initage,  time) - Deltasim(~rent, initage, time).*Hsim(~rent, initage,  time);
...
Acsim(:, :, time)   = Asim(:, :, time) + 1/(1 + p.rl)*Transfer; 
...
for time = 2 : S
  ...
  state              = (1 + p.rl).*Acsim(rent, initage,  time);
  ...
  Attemp             = (1 + p.rl).*Acsim(~rent, initage,  time) - Deltasim(~rent, initage, time).*Hcsim(~rent, initage,  time);
