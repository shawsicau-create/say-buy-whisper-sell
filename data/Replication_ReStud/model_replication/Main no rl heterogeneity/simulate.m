...
state              = (1 + p.rl).*A(:,t);                                                                  % others irrelevant here
...
for t = 2 : T
  ...
  state              = (1 + p.rl).*A(rent, t);                                                                % others don't matter directly
  ...
  state              = [(1 + p.rl).*A(~rent, t) - Delta(~rent, t).*H(~rent, t), O(~rent, t), Th(~rent, t), H(~rent, t)];                     % others don't matter directly
...
err_agg = norm(vec(C(:,t) + H(:, t+1) + A(:, t+1) + (1 + p.rm)*Debt(:, t) + Rent(:,t) - Yh(:,t) - Y(:,t) - (1 + p.rl).*A(:, t) - H(:, t).*(1 - Delta(:,t)) - Debt(:, t+1) + Ftrans(:,t)));
...
% if exist('x', 'var')
% 
%     fprintf('%5.6f  %5.6f  %5.6f  %5.6f  %5.6f  %5.6f  %5.6f %5.6f \n',  [x(:)', err_mom]);
% 
% else
%     
% fprintf('\n');     
