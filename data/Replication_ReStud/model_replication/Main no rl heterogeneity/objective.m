...
p.B          = x(6);
p.rl         = (1 + x(7))^(1/4) - 1;              % interest rate
...
vbarh(:, p.T + 1) = p.pidelta(1)*p.B*(p.wbar + (1 + p.rl).*svbarh(:,1) + (1 - p.Fs - svbarh(:,2).*svbarh(:,3)*(1 + p.rm) - p.delta(1)).*svbarh(:,4)).^(1 - p.sigma)/(1 - p.sigma) + ...
                    p.pidelta(2)*p.B*(p.wbar + (1 + p.rl).*svbarh(:,1) + (1 - p.Fs - svbarh(:,2).*svbarh(:,3)*(1 + p.rm) - p.delta(2)).*svbarh(:,4)).^(1 - p.sigma)/(1 - p.sigma);
...
for t = p.T : -1 : 1
    ...
    vhtemp        = p.pidelta(1)*Vhinterp((1 + p.rl).*svht(:,1) - p.delta(1)*svht(:, 4), ind3h) + p.pidelta(2)*Vhinterp((1 + p.rl).*svht(:,1) - p.delta(2)*svht(:, 4), ind3h);
    
    vrtemp        = Vrinterp((1 + p.rl).*svrt(:,1), ind3r);
