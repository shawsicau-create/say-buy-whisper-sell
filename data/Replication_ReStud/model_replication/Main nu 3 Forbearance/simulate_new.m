...
% With intervention: use solveh which introduces a mortgage forbearance

p.forb          = 0.15;         % don't have to repay more than this fraction of income
...
for time = 1 : S
   ...
   Chct(time)  = sum(vec(Ccsim(:, :, time).*(Hcsim(:, :, time) > 0)))/ sum(vec((Hcsim(:, :, time) > 0)));
...
gains         = max(min((Vnew - Vold)./UCold, 1), 0);  % small fraction due to interpolation error 

owner         = reshape(Hsim(:, :, 2) > 0, 2*N*T, 1); 
borr          = reshape(Osim(:, :, 2) > 0, 2*N*T, 1); 

poor          = reshape(Ysim(:, :, 2) <= prctile(vec(Ysim(:, :, 2)), 25), 2*N*T, 1); 

% Gains from introducing mortgage forbearance policy
...
fprintf('all households                              = %9.0f \n',  round(mean(gains)*12896/10)*10); 
fprintf('homeowners                                  = %9.0f \n',  round(mean(gains(owner))*12896/10)*10); 
fprintf('renters                                     = %9.0f \n',  round(mean(gains(~owner))*12896/10)*10); 
fprintf('poorest 25 percent                          = %9.0f \n',  round(mean(gains(poor & owner(:)))*12896/10)*10); 


fprintf('welfare gains, mean                         = %9.0f %9.0f %9.0f\n',  round([mean(gains),         mean(gains(owner)),         mean(gains(~owner))       ]*12896/10)*10);
fprintf('welfare gains, 10th pctile                  = %9.0f %9.0f %9.0f\n',  round([prctile(gains, 10),  prctile(gains(owner), 10),  prctile(gains(~owner), 10)]*12896/10)*10);
fprintf('welfare gains, 25th pctile                  = %9.0f %9.0f %9.0f\n',  round([prctile(gains, 25),  prctile(gains(owner), 25),  prctile(gains(~owner), 25)]*12896/10)*10);
fprintf('welfare gains, 50th pctile                  = %9.0f %9.0f %9.0f\n',  round([prctile(gains, 50),  prctile(gains(owner), 50),  prctile(gains(~owner), 50)]*12896/10)*10);
fprintf('welfare gains, 75th pctile                  = %9.0f %9.0f %9.0f\n',  round([prctile(gains, 75),  prctile(gains(owner), 75),  prctile(gains(~owner), 75)]*12896/10)*10);
fprintf('welfare gains, 90th pctile                  = %9.0f %9.0f %9.0f\n',  round([prctile(gains, 90),  prctile(gains(owner), 90),  prctile(gains(~owner), 90)]*12896/10)*10);
fprintf('welfare gains, mean, poor owners            = %9.0f \n',  round(mean(gains(poor & owner(:)))*12896/10)*10);
...
Cons   = Csim;                              % - p.phi^(1 + 1/p.gamma)*Csim.^(-p.sigma/p.gamma); 
Consc  = Ccsim;                             % - p.phi^(1 + 1/p.gamma)*Csim.^(-p.sigma/p.gamma); 

dC     = log(Cons(:, 1 : T - 1, 2)./Cons(:, 1 : T - 1, 1));    % eliminate those in last period of life since they die in second period
dCc    = log(Consc(:, 1 : T - 1, 2)./Consc(:, 1 : T - 1, 1));

shck   = log(p.egrid(Esim(:, 1 : T-1, 2))); 

sel    = Hsim(:, 1 : T - 1, 2).*Thsim(:, 1 : T-1).*Osim(:, 1: T - 1, 2) > 0; 

cov(dC(sel), shck(sel))
cov(dCc(sel), shck(sel))
%}
...
% 
% time   = 2;
% happy  = Thcsim(:, :, time) > 0 & Ocsim(:, :, time) > 0 & Dcsim(:, :, time) == 5  & Ocsim(:, :, time + 1) > (1 + p.rm)*Ocsim(:, :, time) - p.mbar; 
% owner  =  Hcsim(:, :, time) > 0; 
% inject = ((Ocsim(:, :, time + 1) - ((1 + p.rm)*Ocsim(:, :, time) - p.mbar)).*Hcsim(:, :, time).*Thcsim(:, :, time)).*happy; 
% 
% fprintf('fraction of homeowners who are affected              = %9.3f\n',  sum(happy(:))/sum(owner(:)));
% fprintf('welfare gains for those affected by policy at t      = %9.0f\n',  round(mean(gains(happy))*12896/10)*10);
% fprintf('average amount received under policy                 = %9.0f\n',  round(mean(inject(happy))*12896/10)*10);
