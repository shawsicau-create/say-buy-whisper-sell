...
fprintf('mortgage debt to income                            = %9.2f \n',  moment_model(4));
% 
% fprintf('\n')
...
%Fraction affected by policy

happy  = Th(:, 1 : end - 1) > 0 & O(:, 1 : end - 1) > 0 & D== 5  & O(:, 2 : end) > (1 + p.rm)*O(:, 1 : end - 1) - p.mbar; 
owner  =  H(:, 1 : end - 1) > 0; 
inject = ((O(:, 2 : end) - ((1 + p.rm)*O(:, 1 : end - 1) - p.mbar)).*H(:, 1 : end - 1).*Th(:, 1 : end-1)).*happy; 
fprintf('\n')

fprintf('fraction homeowners with reduced payments          = %9.2f\n',  sum(happy(:))/sum(owner(:)));
fprintf('mean payment reduction, 2016 USD                   = %9.0f\n',  round(mean(inject(happy))*12896/10)*10);
