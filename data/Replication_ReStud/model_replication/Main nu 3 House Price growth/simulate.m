...
[Lall, Oall, Thall, Hall, V(:,t), Pall(:, 1: 3, t), Vall(:, 1 : 3, t)] = solveh(state, Whinterp, Wrinterp, p, p.thetay(t), 'r', state(:,1), Y(:, t), p.Pt(t), p.Rt(t), Z(:, t));
...
for t = 2 : T
  ...
  tind               = lookup1(p.tgrid, Th(~rent, t), 3);   % no longer on the grid so need to interpolate decision rules
  
  wt                 = (p.tgrid(tind + 1) - Th(~rent, t))./(p.tgrid(tind + 1) - p.tgrid(tind));
...
Rent              = p.Rt(1:end-1)'.*(p.Rt(1:end-1)'/p.alpha).^(-1/p.sigma).*C.*(H(:, 2:end) == 0); 
...
Ftrans   = (H(:, 1 : end - 1) == 0).*(D == 3).*(p.F0m + p.F1m*Debttilde(:, 2:end)) + ...             % mortgage origination cost for renters
           (H(:, 1 : end - 1) >  0).*(D <= 3).*(p.Fs*p.Pt(1:end - 1)'.*H(:, 1 : end - 1)) + ...      % house selling costs for homeowners
...
err_agg = norm(vec(C(:,t) + p.Pt(t)'.*H(:, t+1) + A(:, t+1) + (1 + p.rm)*Debt(:, t) + Rent(:,t) - Yh(:,t) - Y(:,t) - (1 + rl).*A(:, t) - H(:, t).*(p.Pt(t)' - Delta(:,t)) - Debt(:, t+1) + Ftrans(:,t)));

fprintf('Err in Agg Resource Constr                     = %9.2e \n',  err_agg);

id = 1213;

t = 1:70; [C(id, t)' + p.Pt(t).*(H(id, t+1)' - H(id, t)') + Delta(id, t)'.*H(id, t)' + Ftrans(id, t)' + (1 + p.rm)* Debt(id, t)' - Debt(id, t+1)' + A(id, t+1)' - (1 + interest(A(id, t)', p)).*A(id, t)', Y(id, t)' + Yh(id, t)']

t = 1:70; [L(id, t)',  -p.Pt(t).*(H(id, t+1)' - H(id, t)') - Delta(id, t)'.*H(id, t)' - Ftrans(id, t)' - (1 + p.rm)* Debt(id, t)' + Debt(id, t+1)'  + (1 + interest(A(id, t)', p)).*A(id, t)' + Y(id, t)']

t = 1:70; [C(id, t)' + A(id, t+1)' , L(id, t)' + Yh(id, t)']
...
reqpayment      = p.mbar*Th(:, 1:end - 1).*p.Pt(1:end-1)'.*H(:, 1:end - 1).*(O(:, 1:end - 1) > 0).*(D == 5); 
...
moment_model(3)       = mean(vec(p.Pt(1:end-1)'.*H(:, 2 : end)))       /mean(vec(Y))/4;
...
moment_model(41 : 45) = prctile((1 - Th(H >  0).*O(H > 0)).*PH(H > 0)./W(H > 0), [10; 25; 50; 75; 90]);
