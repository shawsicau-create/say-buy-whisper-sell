...
p.alpha      = 0.652986;
p.R          = 0.011387;
p.phi        = 0.932157;                          % productivity non-market
p.F0m        = 0.200509;                          % fixed cost of refinancing
p.B          = 9.727736;                          % bequest motive
p.r1         = 0.300706; 
p.r2         = 9.512026; 
p.rh         = (1 + 0.017297)^(1/4) - 1;          % upper bound on liquid rate

%0.985505  0.652986  0.011387  0.932157  0.200509  9.727736  0.300706 9.512026 0.017297 0.014510  

% Assigned Parameters

p.nu         = 10;                                % parameter in exponential distribution
...
p.P0           = 1;                                         % old house price
p.rm0          = p.rm;                                      % old mortgage interest rate
p.mbar0        = p.rm/(1 - (1 + p.rm)^(-p.D - 0));          % mortgage payment after modification (only applies to old mortgages)

p.P1           = dP;                                        % new house price
p.rm1          = ((1 + p.rm)^4 - dR)^(1/4) - 1;             % interest rate on new mortgage 
p.mbar1        = p.rm1/(1 - (1 + p.rm1)^(-p.D));            % payment on new mortgage
...
p.P1           = 1;                                         % new house price
p.rm1          = ((1 + p.rm)^4 - 0.013125)^(1/4) - 1;       % interest rate on new mortgage 
p.mbar1        = p.rm1/(1 - (1 + p.rm1)^(-p.D));            % payment on new mortgage

%p.rl           = ((1 + p.rl)^4 - 0.013125)^(1/4) - 1; 
%p.rh           = ((1 + p.rh)^4 - 0.013125)^(1/4) - 1;  

p.adjustomega  = (p.mbar0/p.mbar)^0; 
