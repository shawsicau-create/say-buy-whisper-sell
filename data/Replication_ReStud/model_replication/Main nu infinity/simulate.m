...
% LTVtilde          = zeros(size(Debttilde)); 
% LTVtilde(H > 0)   = Debttilde(H>0)./H(H>0);

thetagrid         = lookup1(p.tgrid, Th(:, 2:end), 1);
htemp             = H(:, 2: end); 

% PTItilde          = p.mbar*Debttilde(:, 2:end)./Y; 

newloan           = D == 3 | D == 4; 
LTVbinds          = Th(:, 2:end) == p.thetam & newloan;

PTIuncon          = zeros(2*N, T); 

uncon             = newloan & thetagrid < 5;

PTIuncon(uncon)   = p.mbar*p.tgrid(thetagrid(uncon) + 1).*htemp(uncon)./Y(uncon); 

PTIbinds          = PTIuncon > p.thetay' & ~LTVbinds; 

fractatLTV        = mean(LTVbinds(newloan));
fractatPTI        = mean(PTIbinds(newloan));

%fprintf('\n')
%fprintf('Fraction constrained by: 1. LTV, 2. PTI, 3. unconst    = %9.2f %9.2f %9.2f\n',  [fractatLTV,  fractatPTI, 1 - fractatLTV - fractatPTI]);
%fprintf('\n')
...
moment_model(36 : 40) = prctile(LTV(LTV > 0)./(1 + hpgrowth).^(Age(LTV > 0) - 1), [10; 25; 50; 75; 90]);
...
moment_data = [0.64; 1.45; 1.82; 0.83; 0.46;  0.07; 0.53; 0.15; 0.26; 0.41; 0.20; 0.32; 0.15; 0.23; 2.00; 0.21; 0.044; 0.71; 0.24; 0.11;
...
fprintf('mean wealth                                        = %9.2f %9.2f\n',  [moment_model(2),    moment_data(2)]);
fprintf('mean housing to income                             = %9.2f %9.2f\n',  [moment_model(3),    moment_data(3)]);
fprintf('mean mortgage debt to income                       = %9.2f %9.2f\n',  [moment_model(4),    moment_data(4)]);
...
fprintf('median growth in balance when extract              = %9.2f %9.2f\n',  [moment_model(19),   moment_data(19)]);
fprintf('median change in LTV when extract                  = %9.2f %9.2f\n',  [moment_model(20),   moment_data(20)]);
fprintf('\n')
...
fprintf('mean liquid assets to wealth: extractors           = %9.2f %9.2f\n',  [moment_model(66),   moment_data(66)]);
fprintf('mean liquid assets to wealth: non-extractors       = %9.2f %9.2f\n',  [moment_model(67),   moment_data(67)]);
...
fprintf('median liquid assets to wealth: non-extractors     = %9.2f %9.2f\n',  [moment_model(71),   moment_data(71)]);
...
if 1    % plot life-cycle facts
    ...
    for i = 1 : 12
        ...
        Hage(i) = mean(vec(H(:, 1 + (i - 1)*20 + 1 : i*20))); % end of period
        Wage(i) = mean(vec(W(:, 1 + (i - 1)*20 + 1 : i*20))); 
        Yage(i) = mean(vec(Y(:,     (i - 1)*20 + 1 : i*20)))*4; 
        Dage(i) = mean(vec(Debt(:, 1 + (i - 1)*20 + 1 : i*20))); 
        Hown(i) = mean(vec(H(:, 1 + (i - 1)*20 + 1 : i*20)) > 0); % end of period

    end
    
    Hagedata  = [1.1087; 1.4087; 1.4763; 1.9266; 2.0235; 2.4097; 2.4357; 2.1536; 2.1235; 2.2089; 1.9586; 1.6400];
    Wagedata  = [0.6662; 0.8185; 0.8671; 1.2850; 1.4026; 1.7998; 2.0514; 2.1628; 2.2489; 2.6734; 2.6458; 2.4242];
    Dagedata  = [0.7153; 0.9609; 0.9500; 1.0210; 1.0908; 1.0429; 0.8714; 0.6650; 0.5929; 0.2739; 0.1116; 0.1019];
    Yagedata  = [0.9588; 0.9789; 1.0230; 1.0682; 1.0930; 1.2830; 1.0935; 0.9195; 0.9010; 0.6512; 0.6091; 0.7702];
    Howndata  = [0.4356; 0.5431; 0.5885; 0.6722; 0.7182; 0.7263; 0.7440; 0.7515; 0.7414; 0.7682; 0.7021; 0.5275];   
    
    Aage      = Wage - (Hage - Dage); 
    Aagedata  = Wagedata - (Hagedata - Dagedata); 
    ...
    subplot(2,2,1); 
    plot((27.5:5:82.5)', [Yagedata/mean(Yagedata), Yage/mean(Yage)]); 
    set(get(gcf,'CurrentAxes'),'FontSize',24,'LineWidth',2)
    ...
    subplot(2,2,2); 
    plot((27.5:5:82.5)', [Wagedata./Yagedata, Wage./Yage]); 
    set(get(gcf,'CurrentAxes'),'FontSize',24,'LineWidth',2)
    set(gca,'DefaultTextInterpreter', 'latex','TickLabelInterpreter','latex')
    xlabel('age', 'Interpreter','latex');
    ...
    subplot(2,2,3); 
    plot((27.5:5:82.5)', [Hagedata./Yagedata, Hage./Yage]); 
    set(get(gcf,'CurrentAxes'),'FontSize',24,'LineWidth',2)
    set(gca,'DefaultTextInterpreter', 'latex','TickLabelInterpreter','latex')
    xlabel('age', 'Interpreter','latex');
    ...
    subplot(2,2,4); 
    plot((27.5:5:82.5)', [(Hagedata - Dagedata)./Yagedata, (Hage - Dage)./Yage]); 
    set(get(gcf,'CurrentAxes'),'FontSize',24,'LineWidth',2)
    set(gca,'DefaultTextInterpreter', 'latex','TickLabelInterpreter','latex')
    xlabel('age', 'Interpreter','latex');
    ...
    set(gcf,'DefaultLineLineWidth', 3);
    subplot(2,2,1); 
    plot((27.5:5:82.5)', [Howndata, Hown]); 
    set(get(gcf,'CurrentAxes'),'FontSize',24,'LineWidth',2)
    set(gca,'DefaultTextInterpreter', 'latex','TickLabelInterpreter','latex')
    xlabel('age', 'Interpreter','latex');
...
% Compute cross-section statistics by age and income
...
own  = H(:, date + 1) > 0;                                    % homeowner status at end of period

sel  = Y(:, date) < prctile(vec(Y(:, date)), 100/3); 
h11  = mean(own(sel)); 

sel  = Y(:, date) < prctile(vec(Y(:, date)), 200/3) & Y(:, date) >= prctile(vec(Y(:, date)), 100/3); 
h12  = mean(own(sel)); 

sel  = Y(:, date) >= prctile(vec(Y(:, date)), 200/3); 
h13  = mean(own(sel)); 


% age >= 45 & age < 65 (time >= 80 & time <= 159)
...
hydata = [0.5020837; 0.6760808; 0.7466637]; 
...
hadata = [mean(data(1:3)); mean(data(4:6)); mean(data(7:9))]; 

subplot(1, 2, 2); 
set(gcf,'DefaultLineLineWidth', 3);

bar([1; 2; 3], [hadata, [h1; h2; h3]])
