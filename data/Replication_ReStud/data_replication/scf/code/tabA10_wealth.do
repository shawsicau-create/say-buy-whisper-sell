...
*Fraction borrowers who extract (from our table 1, using PSID data)
preserve
gen res=0.08
...
preserve
replace wealth=wealth/equiv
egen meanpop = wtmean(wealth), weight(wgt)
collapse (mean) wealth (mean) meanpop [weight=wgt] if age_head<=30
gen res=wealth/meanpop
replace res = round(res, 0.01)
gen title="mean wealth (25-30) / mean wealth (all)"
...
outsheet using "`output'/tabA10_wealth.csv", replace c
